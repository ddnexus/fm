...
- Read the small usage message and run it with `~/try-fm run` (Notice that the first run will also build the docker image and setup the docker container HOME)
...
2. In order to make you try FM as it was installed in your own system, the `try-fm` docker container will bind mount your own `/` at `/$(hostname)` and cd you to your mounted host `HOME`. As for any mounted disk that you can write to, __all the changes__ that you may make to your own filesystem from the docker session and from the FM TUI __will be made to your real files__ ! So don't delete anything important!
...
Everything else outside your mounted host filesystem and the docker `~` can be teporarily modified, but will get restored in the next docker session (i.e. the next time you will run `~/try-fm run`).

So to recap the behavior of the filesystem in the docker session:

- `/$(hostname)/*` is your own writable filesystem, so don't do anything stupid with it :)
- `~` is persisted between sessions, so use it to experiment with dot-files. You can reset it with `~/try-fm reset` at any time
- everything else will get restored the next session (i.e. the next time you will run `~/try-fm run`)
